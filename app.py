...
st.markdown("### 🎯 AI 托管预测 (Weighted Calibration)")
...
def auto_tune_model():
    ...
    try:
        # 1. 抓取数据 (扩大量级以确保计算稳定)
        df = yf.download("BTDR", period="2mo", interval="1d", progress=False)
        
        if len(df) < 15: return default_model, "数据不足"
        ...
        # 定义特征：X(开盘涨幅), Y_high(最高价涨幅), Y_low(最低价涨幅)
        ...
        # 3. AI 核心：时间加权回归 (Weighted Regression)
        # 生成权重：最近的数据权重为1.0，最远的数据权重为0.3 (线性衰减)
        # 这使得模型对“最近的市场风格”更敏感，实现自动快速校准
        sample_size = len(x)
        weights = np.linspace(0.3, 1.0, sample_size)
        
        # 加权协方差计算函数
        def weighted_slope(x_arr, y_arr, w_arr):
            w_mean_x = np.average(x_arr, weights=w_arr)
            w_mean_y = np.average(y_arr, weights=w_arr)
            # 协方差与方差的加权计算
            numerator = np.sum(w_arr * (x_arr - w_mean_x) * (y_arr - w_mean_y))
            denominator = np.sum(w_arr * (x_arr - w_mean_x) ** 2)
            slope = numerator / denominator if denominator != 0 else 0
            intercept = w_mean_y - slope * w_mean_x
            return slope, intercept

        # 计算 Beta 和 Intercept
        beta_h, intercept_h_raw = weighted_slope(x, y_high, weights)
        beta_l, intercept_l_raw = weighted_slope(x, y_low, weights)
        
        # 4. 安全边界限制 (防止极端数据导致模型过拟合)
        beta_h = np.clip(beta_h, 0.3, 1.3) # 限制 High 的弹性区间
        beta_l = np.clip(beta_l, 0.4, 1.6) # 限制 Low 的弹性区间
        
        # 5. 模型融合 (70% 历史先验 + 30% 动态加权) -> 增强稳定性
        final_model = {
            "high": {
                "intercept": 0.6*4.29 + 0.4*intercept_h_raw, 
                "beta_open": 0.6*0.67 + 0.4*beta_h, 
                "beta_btc": 0.52
            },
            "low": {
                "intercept": 0.6*-3.22 + 0.4*intercept_l_raw, 
                "beta_open": 0.6*0.88 + 0.4*beta_l, 
                "beta_btc": 0.42
            },
            "beta_sector": 0.25
        }
        return final_model, "自适应校准(AI)"
    except Exception as e:
        return default_model, "默认参数(Err)"
...
def render_ui(data):
    ...
    btc_price = quotes['BTC-USD']['price'] # 获取 BTC 价格
    btdr = quotes['BTDR']
    ...
    # --- 1. 核心指标 (已修改：显示 BTC 价格) ---
    # Card 传入参数: Label, Value(大字), Delta(小字), Delta值(用于颜色)
    ph_btc.markdown(card_html(
        "BTC (全时段)", 
        f"${btc_price:,.2f}",  # 大字显示价格
        f"{btc_chg:+.2f}%",    # 小字显示涨跌幅
        btc_chg
    ), unsafe_allow_html=True)
    ...
    ph_footer.caption(f"Update: {now_ny} ET | Auto-Tuned by AI (Weighted Rolling)")
