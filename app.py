...
# --- 1. 页面配置 & 样式 ---
st.set_page_config(page_title="BTDR Pilot v13.5 Robust", layout="centered")
...
def run_grandmaster_analytics(live_price=None):
    ...
    try:
        ...
        bu_val = boll_u.iloc[-1] if not np.isnan(boll_u.iloc[-1]) else last_close * 1.10
        bl_val = boll_l.iloc[-1] if not np.isnan(boll_l.iloc[-1]) else last_close * 0.90
        bm_val = sma20.iloc[-1] if not np.isnan(sma20.iloc[-1]) else last_close
        atr_val = atr.iloc[-1] if not np.isnan(atr.iloc[-1]) else last_close * 0.05
...
def get_realtime_data():
    # 构造默认兜底字典
    fallback_quotes = {"price": 0, "pct": 0, "prev": 0, "open": 0, "vol": 0, "tag": "ERR", "css": "dot-closed", "is_open_today": False}
    try:
        t = yf.Ticker("BTDR")
        try: 
            ...
            if h.empty: return fallback_quotes, 50, 0.05, pd.DataFrame()
    ...
    except: return fallback_quotes, 50, 0.05, pd.DataFrame()
...
def show_live_dashboard():
    ...
    open_p = quotes.get('open', 0)
    if open_p <= 0: open_p = quotes.get('prev', live_price)
    ...
    # Probability Chart (Optional)
    # st.markdown("### ☁️ 概率推演")
    # ... code for PDF chart ...

st.markdown("### ⚡ BTDR 领航员 v13.5 Robust")
show_live_dashboard()
